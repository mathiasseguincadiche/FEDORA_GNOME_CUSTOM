...
# Documentation and contract tests may name forbidden quirks to explain/reject them.
# Only executable/configuration surfaces are forbidden from containing them.
! grep -RInE --exclude-dir=.git '(i915\.force_probe|xe\.force_probe|selinux=0|enforcing=0|intel_idle\.max_cstate|pcie_aspm=off)' "$ROOT/config" "$ROOT/modules" "$ROOT/scripts" "$ROOT/install.sh" || { echo 'forbidden stability workaround found' >&2; exit 1; }
