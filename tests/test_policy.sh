...
! grep -RInE --exclude-dir=.git --exclude='test_policy.sh' --exclude='test_baseline_contract.sh' '(i915\.force_probe|xe\.force_probe|selinux=0|enforcing=0|intel_idle\.max_cstate|pcie_aspm=off)' "$ROOT" || { echo 'forbidden stability workaround found' >&2; exit 1; }
