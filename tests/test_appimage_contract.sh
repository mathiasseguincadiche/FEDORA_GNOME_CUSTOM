...
manifest=manifests/packages-appimage.txt
flatpaks=manifests/flatpaks-appimage.txt
...
for f in "$manifest" "$flatpaks" "$module" "$doctor" "$runner" "$workflow" docs/APPIMAGE.md; do
  [[ -s "$f" ]] || fail "missing $f"
done
...
for pkg in fuse fuse-libs.x86_64 fuse-libs.i686 fuse3 fuse3-libs.x86_64 fuse3-libs.i686 bsdtar squashfs-tools file desktop-file-utils shared-mime-info xdg-utils; do
  grep -Fxq "$pkg" "$manifest" || fail "$pkg missing from AppImage manifest"
done
grep -Fxq 'it.mijorus.gearlever' "$flatpaks" || fail 'Gear Lever missing from AppImage Flatpak manifest'
