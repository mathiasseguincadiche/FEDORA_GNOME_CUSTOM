...
public_entrypoints=(
  ...
  scripts/gnome/display-repair.sh scripts/gnome/display-watch.sh scripts/gnome/nautilus-prewarm.sh
)
for file in "${public_entrypoints[@]}"; do [[ -f "$ROOT/$file" ]] || { echo "missing public entrypoint: $file" >&2; exit 1; }; [[ -x "$ROOT/$file" ]] || { echo "public entrypoint is not executable: $file" >&2; exit 1; }; bash -n "$ROOT/$file"; done
