...
professional_apps_precheck() {
  ...
  if is_true "${ENABLE_PROFESSIONAL_FLATPAKS:-true}"; then
    ...
    flatpak remotes --columns=name 2>/dev/null | grep -Fxq flathub || {
      log_error APPLICATIONS 'Flathub is required for the professional Flatpak profile'
      return "$EXIT_PRECHECK_FAILED"
    }
  fi
}

professional_apps_plan() {
  cat <<'EOF'
...
professional_apps_apply() {
  ...
  install_manifest_packages APPLICATIONS "$REPO_ROOT/manifests/packages-applications-professional-vendor.txt" || return "$EXIT_APPLY_FAILED"
