...
hardware_topology_plan() { echo 'Certify the MSI B850M MORTAR WIFI topology, UEFI/Secure Boot, AMD virtualization, optional IOMMU observability, Arc B580 ReBAR endpoint and both Crucial T705 endpoints without changing firmware settings.'; }
...
hardware_topology_postcheck() {
  ...
  if is_true "${PLATFORM_REQUIRE_IOMMU_FOR_KVM:-false}"; then find /sys/kernel/iommu_groups -mindepth 1 -maxdepth 1 -type d -print -quit 2>/dev/null | grep -q . || return "$EXIT_POSTCHECK_FAILED"; fi
