...
hardware_storage_postcheck() {
  ...
  local report="$REPORT_ROOT/$RUN_ID-nvme-health.txt" count
  { sudo nvme list; echo; sudo nvme list-subsys; echo; for d in /dev/nvme[0-9]; do [[ -e "$d" ]] && sudo nvme smart-log "$d" || true; done; } > "$report" 2>&1
  ...
  grep -Eqi 'critical_warning[[:space:]]*:[[:space:]]*0|critical_warning[[:space:]]*:[[:space:]]*0x0' "$report" || log_warn HARDWARE 'review NVMe critical warning fields manually'
}
