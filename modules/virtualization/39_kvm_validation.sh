...
kvm_validation_plan() { echo 'Validate AMD KVM, direct qemu:///system operator access, domain capabilities, UEFI/TPM, dedicated T705 pool, isolated NAT, complete CLI stack, two guest profiles and measured I/O tooling.'; }
...
kvm_validation_postcheck() {
  ...
  sudo virsh --connect "${LIBVIRT_URI:-qemu:///system}" pool-info "$pool" | grep -Eq '^State:[[:space:]]+running' || return "$EXIT_POSTCHECK_FAILED"; sudo virsh --connect "${LIBVIRT_URI:-qemu:///system}" net-info "$network" | grep -Eq '^Active:[[:space:]]+yes' || return "$EXIT_POSTCHECK_FAILED"
  ...
  "$REPO_ROOT/diagnostics/kvm-domain-doctor" --quiet || return "$EXIT_POSTCHECK_FAILED"
  for helper in guest/ubuntu-devops/bootstrap-devops.sh guest/ubuntu-devops/verify-devops.sh guest/windows-11/configure-guest-integration.ps1 scripts/kvm/create_ubuntu_devops_vm.sh scripts/kvm/create_windows11_vm.sh scripts/kvm/runtime_certification.sh diagnostics/kvm-io-doctor diagnostics/kvm-domain-doctor; do [[ -r "$REPO_ROOT/$helper" ]] || return "$EXIT_POSTCHECK_FAILED"; done
  printf '%s\n' 'KVM CONTRACT READY — capabilities and operator access are certified; guest XML/agents/live traffic are certified after guest creation'
}
