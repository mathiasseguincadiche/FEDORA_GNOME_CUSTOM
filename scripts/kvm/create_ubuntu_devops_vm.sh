...
Usage:
  create_ubuntu_devops_vm.sh \
    --cloud-image /path/to/ubuntu-26.04-server-cloudimg-amd64.img \
    [--ssh-key /path/to/key.pub] \
    [--sha256sums /path/to/SHA256SUMS] \
    [--signature /path/to/SHA256SUMS.gpg] \
    [--canonical-key-file /path/to/canonical-cloud-image-signing-key.asc]

When cloud-image verification is required, SHA256SUMS and SHA256SUMS.gpg default
...
while (($#)); do
  case "$1" in
    ...
    --canonical-key-file) canonical_key_file="${2:-}"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
...
for cmd in virsh virt-install qemu-img cloud-localds openssl base64 findmnt restorecon; do
  need "$cmd"
done

if [[ "${UBUNTU_SERVER_CLOUD_IMAGE_VERIFICATION_REQUIRED:-true}" == true ]]; then
  verifier="$REPO_ROOT/scripts/kvm/verify_ubuntu_cloud_image.sh"
  [[ -r "$verifier" ]] || fail "cloud-image verifier missing: $verifier"
  image_dir="$(cd "$(dirname "$cloud_image")" && pwd)"
  [[ -n "$sha256sums" ]] || sha256sums="$image_dir/${UBUNTU_SERVER_CLOUD_IMAGE_SUMS_FILENAME:-SHA256SUMS}"
  [[ -n "$signature" ]] || signature="$image_dir/${UBUNTU_SERVER_CLOUD_IMAGE_SIGNATURE_FILENAME:-SHA256SUMS.gpg}"
  [[ -r "$sha256sums" ]] || fail "authenticated image policy requires readable SHA256SUMS: $sha256sums"
  [[ -r "$signature" ]] || fail "authenticated image policy requires readable SHA256SUMS.gpg: $signature"
  ...
  if [[ -n "$canonical_key_file" ]]; then
    [[ -r "$canonical_key_file" ]] || fail "Canonical key file not readable: $canonical_key_file"
    verify_args+=(--key-file "$canonical_key_file")
  fi
  bash "$verifier" "${verify_args[@]}" || fail 'Canonical cloud-image verification failed'
fi
...
printf 'Canonical image authenticity was verified before disk creation.\n'
