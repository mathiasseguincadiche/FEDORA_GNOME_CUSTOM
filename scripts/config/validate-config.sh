...
CONFIG_LITERAL_DOLLAR_PAREN="\$("
CONFIG_LITERAL_HOME="\$HOME/"
CONFIG_LITERAL_BRACED_HOME="\${HOME}/"
...
safe_value() {
  local raw="$1" first last body
  [[ "$raw" != *"$CONFIG_LITERAL_DOLLAR_PAREN"* && "$raw" != *'`'* ]] || return 1
  ...
  if ((${#raw} >= 2)); then
    ...
    if [[ "$first" == '"' && "$last" == '"' ]]; then
      body="${raw:1:${#raw}-2}"
      # Keep the grammar deliberately simple: one complete quoted scalar.
      # Shell metacharacters such as <Super>d are literal and safe inside it,
      # but nested quote boundaries are not accepted.
      [[ "$body" != *'"'* ]]
      return
    fi
    if [[ "$first" == "'" && "$last" == "'" ]]; then
  ...
  # Unquoted scalars must be one plain shell word. Control/redirection/pipeline
  # operators are forbidden because KEY=>file is executable shell redirection.
  [[ "$raw" != *'"'* && "$raw" != *"'"* && "$raw" != *';'* && "$raw" != *'<'* && "$raw" != *'>'* && "$raw" != *'|'* && "$raw" != *'&'* && ! "$raw" =~ [[:space:]] ]]
}
...
classify_value() {
  ...
  if [[ "$value" == /* || "$value" == "$CONFIG_LITERAL_HOME"* || "$value" == "$CONFIG_LITERAL_BRACED_HOME"* ]]; then printf 'path\n'; return 0; fi
  printf 'string\n'
}
...
validate_class_value() {
  ...
  case "$expected" in
    repository)
      ...
      ;;
    optional_path)
      [[ -z "$value" || "$value" == /* || "$value" == "$CONFIG_LITERAL_HOME"* || "$value" == "$CONFIG_LITERAL_BRACED_HOME"* ]] || return 1
      ;;
    device_or_auto)
