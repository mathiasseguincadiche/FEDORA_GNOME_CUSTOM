...
baseline_find_expected_gpu() {
  local dev vendor device driver expected_vendor expected_device
  expected_vendor="$(normalize_hex "${EXPECTED_GPU_PCI_VENDOR:-8086}")"; expected_device="$(normalize_hex "${EXPECTED_GPU_PCI_DEVICE:-e20b}")"
  for dev in /sys/bus/pci/devices/*; do
    ...
    driver=unbound; [[ -L "$dev/driver" ]] && driver="$(basename "$(readlink -f "$dev/driver")")"
    printf '%s|%s\n' "${dev##*/}" "$driver"; return 0
  done
  return 1
}
...
baseline_gpu_inventory() {
  local found driver
  found="$(baseline_find_expected_gpu 2>/dev/null || true)"
  [[ -n "$found" ]] || { printf 'missing\n'; return; }
  driver="${found#*|}"
...
baseline_nvme_inventory() {
  ...
  for d in /sys/class/nvme/nvme*; do
    [[ -d "$d" ]] || continue
    printf 'model=%s|serial=%s|firmware=%s\n' "$(baseline_hw_value "$d/model")" "$(baseline_hw_value "$d/serial")" "$(baseline_hw_value "$d/firmware_rev")"
  done | sort
}
...
baseline_write_evidence() {
  ...
  local name="$1" status="$2" detail="${3:-}" path
  baseline_ensure_dirs; path="$(baseline_evidence_dir)/$name.ok"
...
baseline_evidence_valid() {
  ...
  local name="$1" file; file="$(baseline_evidence_dir)/$name.ok"
  [[ -s "$file" ]] || return 1
  grep -Fxq 'status=PASS' "$file" || return 1
  grep -Fxq "fingerprint=$(baseline_fingerprint)" "$file" || return 1
  case "$name" in memory-5600|memory-6000|nvme-root|nvme-data) grep -Eq '^detail=.*automated=true.*sha256=[0-9a-f]{64}' "$file" ;; esac
}
...
baseline_certification_valid() {
  runtime_is_baremetal || return 1
  local marker; marker="$(baseline_certification_path)"
  [[ -s "$marker" ]] || return 1
